...
export const getPlayerCard = (player) => {
  ...
  // Your code here
  if(player===null || player===undefined || player.name===undefined) return null;
  const name = player.name;
  const runs = player.runs;
  const balls = player.balls;
  const totalRuns = player.totalRuns;
  const innings = player.innings;
  const notOuts = player.notOuts;
  const runsConceded = player.runsConceded;
  const overs = player.overs;
